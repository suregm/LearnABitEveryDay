...
:: 3、psexec.exe \\%ip% -u %username% -p %password% cmd /c open.bat


net use \\%远程计算机IP%/ipc$ "%远程密码%" /user:%远程用户名%
at \\%远程用户名% %要自动登录的时间% "d:\remote.cmd"
...
所有的pstools都支持IPC$，一旦IPC$共享是连接的就不用输入-u 和-p这两个参数。

参数：
-u 后面跟用户名 -p后面是跟密码的，如果建立ipc$连接后这两个参数则不需要。（如果没有-p参数，则输入命令后会要求你输入密码）
-c [[路径]文件名]:拷贝文件到远程机器并运行（注意：运行结束后文件会自动删除）
-d 不等待程序执行完就返回，（比如要让远程机器运行tftp服务端的时候使用，不然psexec命令会一直等待tftp程序结束才会返回）
-i 在远程机器上运行一个名为psexesvc进程，(-i 0----------”0“为远程桌面的会话ID）

rem 假设我在远程机器ip有一个账号，账号名是：abc 密码是：123
rem 比如想要用telnet一样在远程系统上执行命令可以打：(远程系统要开admin$）
psexec \\远程机器ip -u abc -p 123 cmd
rem 如果想要远程机器执行本地c:\srm.exe文件可以打：
psexec \\远程机器ip -u abc -p 123 -c c:\srm.exe
rem 如果想要让远程机器执行本地上tftp服务端，(假设tftp服务端在本地c:\tftp32.exe)，可以打：
psexec \\远程机器ip -u abc -p 123 -c c:\tftp32.exe -d
rem 以下命令可在 \\marklap上启动交互式命令提示窗口：
psexec \\marklap cmd
rem 此命令通过 /all 开关在远程系统上执行 IpConfig，并在本地显示输出结果：
psexec \\marklap ipconfig /all
rem 此命令将程序 test.exe 复制到远程系统，并以交互方式执行此程序：
psexec \\marklap -c test.exe
rem 如果远程系统中已经安装的程序不在系统路径中，请指定该程序的完整路径：
psexec \\marklap c:\bin\test.exe
rem 在系统帐户中以交互方式运行 Regedit，以便查看 SAM 和 SECURITY 注册表项的内容：
psexec -i -d -s c:\windows\regedit.exe
rem 要以受限用户权限运行 Internet Explorer，请使用此命令：
...
echo 你现在可以用%ser%,密码%psd%来远程控制这台计算机了
set tm1=%time:~0,2%
set tm2=%time:~3,2%
set tm3=%time:~6,2%
